...
__STL__ - это часть стандартной библиотеки С++, которая содержит набор готовых шаблонов контейнеров (например: std::vector, std::array), алгоритмов и итераторов. Изначально была сторонней разработкой __Hewlett-Packard__. __Архитектура STL__ была разработана __Александром Степановым__ и __Менг Ли__.
...
__Контейнерный класс (класс-контейнер, контейнер)__ - шаблонный класс, предназначенный для хранения и организации элементов заданного типа.

Контейнеры STL делятся на три основные категории:
* __Последовательные__ - контейнеры, элементы которых находятся в последовательности
  * [std::array]()
  * [std::vector]()
  * [std::list]()
  * [std::deque]()
  * [std::forward_list]()
  * [std::basic_string]()
* __Ассоциативные__ - контейнеры, которые автоматически сортируют все свои элементы (по умолчанию используя оператор <)
  * [std::set]()
  * [std::multiset]()
  * [std::map]()
  * [std::multimap]()
* __Адаптеры__ - уже существующий контейнер, который приспособили для конкретной задачи (например, используя методы предоставленные __std::list__ можно сделать __std::stack__). Самое интересное, что можно самому выбирать какой контейнер должен использовать адаптер
  * [std::stack]()
  * [std::queue]()
  * [std::priority_queue]()
...
__std::vector__ - это динамический массив, который может сам управлять выделенной себе памятью.

__Заголовочный файл:__ ```#include <vector>```
	
| Инициализация вектора ||
| - | - |
| Без указания длины | ```std::vector<int> v;``` |
| С указанием длины (дополнительно инициализирует нулями) | ```std::vector<int> v(n);``` |
| Проинициализировать вектор длины n значением 5 | ```std::vector<int> v(n, 5);``` |
| Список инициализаторов | ```std::vector<int> v = {10, 8, 5, 2};``` |
| Uniform-инициализация (С++11) | ```std::vector<int> v {10, 8, 5, 2};``` |
| Двумерный вектор | ```vector <vector<int>> v;``` |
| Двумерный вектор mxn | ```std::vector<std::vector<int>> v(m, std::vector<int>(n));``` |
| Присвоить вектору значение (С++11) | ```v = {1, 2, 3, -1}``` |
	
| Доступ к элементам ||
| - | - |
| Без проверки диапазона (вернёт 0 или ошибку если выйти за диапазон) | ```v[i]``` |
| С проверкой диапазона (кинет исключение std::out_of_range), соответственно он медленее чем [] из-за проверок | ```v.at(i)``` |
	
| Методы ||
| - | - |
| Поместить элемент в конец вектора | ```v.push_back(i);``` |
| Удалить последний элемент (ничего не возвращает) | ```v.pop_back();``` |
|  | ```v.insert()``` |
|  | ```v.erase()``` |
| Проверка на пустоту | ```v.empty()``` |
| Получить длину вектора | ```v.size();``` |
| Изменить длину вектора | ```v.resize(i);``` |
| Очистить вектор | ```v.clear();``` |
| Емкость вектора | ```v.capacity();``` |
| Явно указать емкость вектора (можно только увеличить текущую емкость) | ```v.reserve(n);``` |
| Подогнать емкость под размер (освободить незанятую элементами память) (с++11) | ```v.shrink_to_fit()``` |

Понятно что для каждого нового элемента заного создавать динамический массив с размером на 1 больше не выгодно. Поэтому место выделяется под несколько элементов сразу (зависит от коэффициента который зависит от реализации). Соответственно:
* __size__ - количество элементов, лежащих в векторе
* __capacity__ - количество элементов лежащих в векторе + сколько еще свободных ячеек до перевыделения памяти. Тоесть текущая длина динамического массива
